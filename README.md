...
| オプション | 説明 |
| --- | --- |
| （なし） | テキスト入力のみのモード。LLMはGPT固定・音声合成なし。生徒に配るPC向けのデフォルト |
| `--speech` | 音声入出力（マイク認識・読み上げ）とAI／音声合成エンジンのカスタマイズを有効にする |
| `--admin` | 「⚙️ APIキー設定」画面とAPIキー登録APIを有効にする（生徒に配るPCでは付けない） |

- `--speech` を付けない場合、`/api/tts`（音声合成）は403エラーになります
- `--admin` を付けない場合、`/settings` の設定画面とAPIキー関連APIは403エラーになります
- 両方付けると、従来通り「使うAI」「モデル」「音声合成エンジン」を画面で自由に選べるフルモードになります

例：講師・TA用のPCでフル機能を使う場合
...
1. （`--admin` 起動時のみ）画面右上の「⚙️ APIキー設定」からAPIキーを登録（内蔵簡易AIだけ使うなら不要）
2. （`--speech` 起動時のみ）左パネルで「使うAI」「モデル」「音声合成エンジン」を選ぶ
3. 「プロンプト」欄でAIの性格を自由に書き換える
4. （`--speech` 起動時のみ）🎤 マイクONボタンを押して話しかける（またはテキスト入力欄から送信）
